...
for (let i = 1; i <= 9; i++) {
  ...
  if (jugadorActual === "Jugador 1") {
    posicion = readlineSync.question(`${jugadorActual}, introduce la posición (ejemplo: A1): `);
    if (posicion[0] === 'A') {
      columna = 0;
    ...
    fila = parseInt(posicion[1]) - 1;
  } else if (jugadorActual === "Jugador 2") {
    let ganandoXhorizontal = [[' X |', ' X |', ' -'], [' - |', ' X |', ' X'], [' X |', ' - |', ' X']];
    for (let i = 0; i < tablero.length; i++) {
      if (ganandoXhorizontal.includes(tablero[i])) {
        fila = i;
      } else {
